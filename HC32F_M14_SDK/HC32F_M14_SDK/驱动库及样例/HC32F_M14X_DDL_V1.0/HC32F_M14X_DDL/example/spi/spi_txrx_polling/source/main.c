...
int32_t main(void)
{
    ...
    stcConfigRx.u32BaudRate = SPI_BAUDRATE;
    ...
    while (u32offset < strlen((const char* )u8TxBuff))
    {
        /* tx a byte. */
        ...
        if(Ok == ret)
        {
            u8RxBuff[u32offset] = (u16Data & 0xff);
            u32offset++;
        }
    }
