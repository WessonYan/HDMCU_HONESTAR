...
/** \file main.c
 **
 ** \brief The example for adc result comparison demonstration.
...
static void ResultCmpIrqHandler(void)
{
    Adc_CmpDisable(ADC_UNIT, AdcCmpResult);
    Adc_IrqDisable(ADC_UNIT, AdcIrqResultCmp);
    Adc_ClearIrqFlag(ADC_UNIT, AdcIrqResultCmp);
...
void Adc_InitConvConfig(void)
{
    ...
    stc_adc_result_cmp_cfg_t stcAdcResultCmpCfg;
    ...
    DDL_ZERO_STRUCT(stcAdcResultCmpCfg);
    DDL_ZERO_STRUCT(stcAdcScanCfg);
    DDL_ZERO_STRUCT(stcAdcSampTimeCfg);
    DDL_ZERO_STRUCT(stcAdcIntrpt);

    /* Configure result comparison. */
    stcAdcResultCmpCfg.bCmpAllChn = FALSE;
    stcAdcResultCmpCfg.enCmpChn = AdcChn10;
    stcAdcResultCmpCfg.enCmpCon = AdcResultCmpCondGreatOrEqual;
    Adc_ResultCmpThrCfg(&stcAdcResultCmpCfg, 0x800);
    ...
    stcAdcIntrpt.bIrqEnResult = TRUE;
    stcAdcIntrpt.stcIsr.pfnIsrResult = ResultCmpIrqHandler;
    ...
    stcAdcInitCfg.pstcAdcResultCmpCfg = &stcAdcResultCmpCfg;
