...
#define TestIOFSET0 bM0P_FASTIO_FSET6_P60
...
int32_t main(void)
{
    ...
    /* Initialize fast gpio for output */
    FGpio_InitIO(T1_PORT,T1_PIN,GpioDirOut);

    while (1)
    {
        /* if use FastIO "bit-band" method, write 0xFF to set High and write
           0x00 to set Low. */
        TestIOFSET0 = 0;
        TestIOFSET0 = 0xFF;
        TestIOFSET0 = 0;
