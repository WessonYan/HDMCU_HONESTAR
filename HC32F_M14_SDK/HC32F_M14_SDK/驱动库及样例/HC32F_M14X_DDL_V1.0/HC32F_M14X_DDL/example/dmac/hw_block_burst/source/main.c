...
/** \file main.c
 **
 ** \brief This sample demonstrates hardware block & burst transfer function of
 **        Dmac
 **
 **   - 2017-6-7  1.0  Zhangxl First version for Device Driver Library of Dmac
 **     hardware block & burst transfer samples.
 **
 ******************************************************************************/
 ...
 ******************************************************************************/
#define DMAC0_BC        5               ///< DMAC Ch.0 block count
#define DMAC0_TC        3               ///< DMAC Ch.0 transfer count

#define DMAC1_BC        5               ///< DMAC Ch.1 block count
#define DMAC1_TC        3               ///< DMAC Ch.1 transfer count

#define CT_RL_CYCLE     100             ///< CT reload mode cycle setting
...
/**
 *******************************************************************************
 ** \brief  Main function of Dmac hardware block and burst transfer
 **
 ** \note This sample uses DMA Ch.0 hardware block transfer, DMA Ch.1 in
 **       hardware burst transfer.
 **
 ******************************************************************************/
int32_t main(void)
{
    /* DMA Ch.0, hardware block transfer */
    stc_dma_config_t stcDma0Cfg, stcDma1Cfg;
    stc_dma_irq_sel_t stcIrqSel;
    stc_dma_irq_cb_t stcIrqCb;

    stc_ct_rt_config_t stcCtRtCfg;
    stc_rt_irq_en_t stcCtRtIrqEn;
    stc_rt_irq_cb_t stcCtRtIrqCb;

    uint8_t i = 0;
    uint8_t u8CtTriggerCnt = 0;
    enDmaTransStat = trans_idle;

    /* config CT ch.0 as RT one-shot mode */
    DDL_ZERO_STRUCT(stcCtRtCfg);
    DDL_ZERO_STRUCT(stcCtRtIrqEn);
    DDL_ZERO_STRUCT(stcCtRtIrqCb);
    stcCtRtIrqEn.bRtUnderflowIrq = TRUE;

    stcCtRtCfg.enMode = RtOneshot;
    stcCtRtCfg.enExtTrig = RtExtTiggerDisable;
    stcCtRtCfg.enOutputPolarity = RtPolarityLow;
    stcCtRtCfg.enPres = RtPresNone;
    stcCtRtCfg.enSize = RtSize16Bit;
    stcCtRtCfg.bTouchNvic = FALSE;
    Ct_Rt_Init(CtChannel0, &stcCtRtCfg);
    Ct_Rt_EnableIrq(CtChannel0, RtUnderflowIrq);

    /* Write cycle value */
    Ct_Rt_WriteCycleVal(CtChannel0, CT_RL_CYCLE);
    ...
    /* 1st transfer gap */
    while (CT_RL_CYCLE != M0P_CT0_RT->TCNT)
    {
    }
    u8CtTriggerCnt++;
    while (1)
    {
        if (TRUE == Ct_Rt_GetIrqFlag(CtChannel0, RtTrigIrq))
        {
            Ct_Rt_ClrIrqFlag(CtChannel0, RtTrigIrq);
            /* trigger singal in transfer gap, CT underflow flag may cleared
               by DMA*/
            Ct_Rt_EnableSwTrig(CtChannel0);
            while (CT_RL_CYCLE != M0P_CT0_RT->TCNT)
            {
            }
            if (TRUE == Ct_Rt_GetIrqFlag(CtChannel0, RtUnderflowIrq))
            {
                Ct_Rt_ClrIrqFlag(CtChannel0, RtUnderflowIrq);
                break;
            }else
            {
                u8CtTriggerCnt++;
    ...
    /* DMA Ch.1, hardware burst tranfer */
    ...
    Dma_Enable();
    /* one trigger to complete transfer in burst mode */
    Ct_Rt_EnableSwTrig(CtChannel0);
