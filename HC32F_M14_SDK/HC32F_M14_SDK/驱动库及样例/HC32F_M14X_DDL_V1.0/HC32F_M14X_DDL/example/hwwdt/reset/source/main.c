...
/** \file main.c
 **
 ** \brief This example shows how to use the reset function of the hardware
 **      watchdog timer.
 **
...
int32_t main(void)
{
    ...
    if(TRUE == m_stcResetCause.bHwwdt)
    {
        while(1)
        {
            Led_On();  /* Led on */
    ...
    /* Initialize hwwdt configuration structure */
    stcHwwdtConfig.u32LoadValue = 0x0FFF;
    stcHwwdtConfig.bResetEnable = TRUE;   /* Enables Hardware watchdog timer reset */
    stcHwwdtConfig.pfnHwwdtIrqCb = HwwdtIrqCb;

    /* Initialize hardware watchdog timer */
    if (Ok != Hwwdt_Init(&stcHwwdtConfig))
    {
      while(1)
      {
          /* nop */
