...
/** \file main.c
 **
 ** \brief The example for RT function which use io mode7 demonstration
 **
...
int32_t main(void)
{
    ...
    Ct_ConfigIOMode(CT_CH, CtIoMode7);

    /* Set Composite Timer  IO port */
    Gpio_SetFunc_TIOA2_0_OUT(0u);
    Gpio_SetFunc_TIOA3_1_OUT(0u);

    Gpio_SetFunc_TIOB2_0_IN(0u);  /* Use to external trigger */
    ...
    Ct_Rt_WriteCycleVal(CT_CH + 1u, (CT_RT_CYCLE+1)*2-1);
    ...
    /* Wait TIOB2_0 falling edge to trigger */
